...
using assms proof(induction t arbitrary: p rule: is_heap.induct)
  ...
  moreover have
    ...
    using Node.IH(1, 2) Node.prems distinct_rev p_defs by blast+
  ultimately have
...
using assms proof(induction x arbitrary: t)
  ...
  then show ?case
    using \<open>set (foldr (\<lambda>(k, p). bst_insert k p) xs \<langle>\<rangle>) = list.set xs\<close>
...
proof
  ...
  then show "search k t \<Longrightarrow> \<exists>p. (k, p) \<in> set t"
    by auto
next
  show "\<exists>p. (k, p) \<in> set t \<Longrightarrow> search k t"
